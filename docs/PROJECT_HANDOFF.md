...
- These field-level outputs come from one documentation renderer, `scripts/build_schema.cjs`; its resolved schema is also saved in `schema_diagrams/SCHEMA_DATA.json`. They contain 91 tables, 1,638 columns, and 464 FKs after the device-scope additions. Type/nullability clarifications and constraints not expressible in basic DBML are documented in `FULL_DATABASE_SCHEMA.md`.
