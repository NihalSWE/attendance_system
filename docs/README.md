...
- **Field contract:** [Model field dictionary](MODEL_FIELD_DICTIONARY.md) — proposed fields for all 83 models, with Django field names/types, FK/O2O/M2M relations, nullability, status choices, deletion behavior, and important constraints. This is documentation, not model code.
- **Database-wide map:** [Proposed PostgreSQL schema](DATABASE_SCHEMA.md) — ER diagrams, all 83 relational tables, 5 implicit M2M junction tables, tenant ownership paths, critical constraints, indexes, idempotency, retention, and scaling notes.
