...
def _placement(device):
    ...
    physically scans. Department and designation must already exist in that
    branch: inventing organisation structure is HR's decision, not a
    ...
    designation = (
        Designation.objects.filter(department=department).order_by("pk").first()
    )
